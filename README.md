...
`tools/list` at `https://gateway.pipeworx.io/data-gr/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_gr_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"tourism"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_gr_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```
ask_pipeworx({ question: "your question about Data Gr data" })
```
